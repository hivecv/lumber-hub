...
async def add_device_logs(device_uuid: str, message: schemas.LogMessage, current_user: schemas.User = Depends(get_current_active_user), db=Depends(get_db)):
    for device in current_user.devices:
        if device.device_uuid == device_uuid:
...
async def fetch_device_logs(device_uuid: str, current_user: schemas.User = Depends(get_current_active_user), db=Depends(get_db)):
    for device in current_user.devices:
        if device.device_uuid == device_uuid:
